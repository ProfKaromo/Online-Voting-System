...
$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
	...
	if($_FILES["fileuploaded"]["size"]>50000){
		?>
			...
			window.location.href='apply_general_secretary_fail.php?fail';
			</script>
			<?php
		$uploadOk=0;
	}
	...
	if(empty($reg_no) || empty($fname) || empty($sname)|| empty($year)||empty($reason) || empty($image_file)||empty($image_loc)) 
	{
				...
				echo "<script type=\"text/javascript\">  window.open('http://127.0.0.1/vote/apply_general_secretary_fail.php', '_self')  </script>";
...
VALUES ('$reg_no','$fname','$sname','$year','secretary','$reason','$image_file','$image_loc')";
